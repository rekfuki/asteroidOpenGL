...
void main() {
    float dist = distanceToOctave(distance(view_position, fs_input.frag_pos), main_asteroid_octaves_factor);
    vec3 norm = recomputeNormals(fs_input.original_pos, fs_input.original_normal, dist, main_asteroid_frequency, 1);
    ...
    float shadow = compute_shadows(fs_input.frag_pos_light_space);                      
