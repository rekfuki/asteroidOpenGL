...
Icosphere::Icosphere(float radius, int sub) : radius(radius), subdivision(sub), interleavedStride(32) {
	attribute_v_coord = 0;
	attribute_v_tex = 1;
	attribute_v_normal = 2;

	buildVerticesSmooth();
	glGenBuffers(1, &icosphereBufferObject);
	glBindBuffer(GL_ARRAY_BUFFER, icosphereBufferObject);
...
void Icosphere::draw(int draw_mode) const {
	int stride = getInterleavedStride();
	glBindBuffer(GL_ARRAY_BUFFER, icosphereBufferObject);
	glVertexAttribPointer(attribute_v_coord, 3, GL_FLOAT, GL_FALSE, stride, (void*)0);
	glEnableVertexAttribArray(attribute_v_coord);

	glVertexAttribPointer(attribute_v_normal, 3, GL_FLOAT, GL_FALSE, stride, (void*)(sizeof(float) * 3));
	glEnableVertexAttribArray(attribute_v_normal);

	glVertexAttribPointer(attribute_v_tex, 2, GL_FLOAT, GL_FALSE, stride, (void*)(sizeof(float) * 6));
	glEnableVertexAttribArray(attribute_v_tex);
	...
	glDrawElements(GL_PATCHES, getIndexCount(), GL_UNSIGNED_INT, (void*)0);
}
